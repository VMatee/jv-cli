    ...
    for path in (root / ".state/runs").glob("*/session.lock"):
        ...
        try:
            try:
                ...
                raise JvError(
                    "An active JV CLI session was detected. Exit it before updating/uninstalling"
                ) from None
        finally:
            os.close(fd)
...
def _excluded(relative: Path) -> bool:
    if any(
        part in EXCLUDED_DIRS or part.startswith("staging")
    ...
    return (
        ...
        or ("session" in name.lower() and "test" not in name.lower())
    )
...
def manifest(root: Path) -> dict[str, str]:
    ...
    if not entries or actual != set(entries):
        detail = (
            sorted(actual - set(entries))
            or sorted(set(entries) - actual)
            or ["empty manifest"]
        )
...
def check_prerequisites(no_engine: bool = False) -> None:
    ...
    if sys.version_info < (3, 10):
        errors.append(
            "Python 3.10 or later is required (Ubuntu: sudo apt install python3)"
        )
    ...
    if shutil.which("node"):
        try:
            ...
            errors.append(
                "Could not verify Node.js version; Node.js 18 or later is required"
            )
    if errors:
        raise JvError(
...
def _install_engine(root: Path, no_engine: bool, offline: bool) -> None:
    ...
    env = {
        ...
        if k in ("PATH", "LANG", "LC_ALL", "USER", "LOGNAME", "SHELL")
    }
    env.update(
        HOME=str(private_dir(cache / "install-home")),
        CODEX_HOME=str(private_dir(cache / "engine-check")),
        TMPDIR=str(private_dir(cache / "tmp")),
        XDG_CACHE_HOME=str(cache),
        NPM_CONFIG_CACHE=str(private_dir(cache / "npm")),
        NPM_CONFIG_UPDATE_NOTIFIER="false",
        NPM_CONFIG_USERCONFIG=str(cache / "npmrc"),
        NPM_CONFIG_GLOBALCONFIG=str(cache / "global-npmrc"),
    )
    for name in ("npmrc", "global-npmrc"):
        atomic_write(cache / name, "")
    lock_fd = os.open(
        root / ".state/install.lock", os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600
    )
    try:
        ...
        if version == ENGINE_VERSION:
            print(
                f"Pinned engine {ENGINE_VERSION} is already installed; no download needed"
            )
            return
        ...
        try:
            ...
            print(
                f"Installing pinned engine {ENGINE_VERSION} inside {root / 'runtime'}",
                flush=True,
            )
            command = [
                "npm",
            ...
            if result.returncode:
                raise JvError(
                    "Local engine installation failed. The previous runtime was preserved"
                )
...
def _copy_release(source: Path, destination: Path) -> None:
    ...
    if destination.exists():
        ...
        if any(child.name != ".state" for child in children):
            raise JvError(
                f"Refusing to install over an unrecognized directory: {destination}"
            )
...
def _install_launcher(root: Path, home: Path) -> Path:
    ...
    if launcher.exists() and not launcher.is_symlink():
        raise JvError(
            f"Refusing to replace an existing non-symlink launcher: {launcher}"
        )
...
def _path_notice(home: Path, add_path: bool) -> None:
    ...
    path_entries = [
        Path(p).expanduser() for p in os.environ.get("PATH", "").split(os.pathsep) if p
    ]
    ...
    if not add_path:
        ...
        print(
            "Re-run ./install.sh --add-path only if you want JV CLI to add this line to ~/.bashrc."
        )
        return
    ...
    if PATH_LINE not in existing.splitlines():
        ...
        print(
            f"Added JV CLI PATH entry to {bashrc}; open a new shell or source it explicitly."
        )
...
def install(
    ...
    *,
    ...
    if os.geteuid() == 0:
        raise JvError(
            "Do not install with sudo or as root. Run ./install.sh as your normal Ubuntu user"
        )
    ...
    if portable:
        ...
        print(
            f'Run directly: "{root / "jvcli"}" doctor\nOr: source "{root / "activate.sh"}"'
        )
    else:
    ...
    if no_engine:
        print(
            "Engine setup skipped. Coding sessions require the pinned engine before use."
        )
    return 0
...
def upgrade(source: Path, destination: Path) -> int:
    ...
    fd = os.open(
        destination / ".state/install.lock",
    ...
    try:
        ...
        try:
            ...
            raise
        print(
            f'Upgraded source to {VERSION}. Runtime, account settings, sessions and caches were preserved.\nPrevious source backup: {backup}\nRun: "{destination / "install.sh"}"'
        )
        return 0
    finally:
        os.close(fd)
...
def uninstall(
    home: Path | None = None, *, keep_state: bool = False, yes: bool = False
) -> int:
    home = no_symlink_path(home or Path.home())
    root = no_symlink_path(home / ".local/share/jv-cli")
    ...
    if launcher.is_symlink() or launcher.exists():
        if (
            ...
            or launcher.resolve() != (root / "bin/jvcli").resolve()
        ):
            raise JvError(f"Refusing to remove an unexpected launcher: {launcher}")
    idle(root)
    print(
        f"WARNING: uninstall will remove JV CLI runtime, caches, and source under {root}."
    )
    print(
        "Your projects are never removed. Local account/session state will be preserved."
        if keep_state
        else "Local account settings and saved sessions will also be removed."
    )
    if not yes:
        if (
            not sys.stdin.isatty()
            or input("Type REMOVE to continue: ").strip() != "REMOVE"
        ):
            raise JvError(
                "Uninstall cancelled; use --yes only after reviewing the warning"
            )
    saved_state = None
...
def main() -> int:
    ...
    up.add_argument(
        "destination", type=Path, nargs="?", default=Path.home() / ".local/share/jv-cli"
    )
