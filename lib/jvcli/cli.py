...
from .structured import STRUCTURED_AGENT_INSTRUCTIONS
...
STATE_DIR = (
    Path(os.environ.get("JVCLI_HOME", str(APP_ROOT / ".state"))).expanduser().absolute()
)
...
def _load_disk_config():
    ...
    if set(config) - allowed:
        raise JvError(
            "config.json contains unknown fields; only base_url and username are allowed. Never store passwords there"
        )
...
def _save_disk_config(config):
    ...
    atomic_write(
        CONFIG_PATH,
        json.dumps(
            {k: v for k, v in config.items() if k in ("username", "base_url")}, indent=2
        )
        + "\n",
    )
...
def _resolve_account(username=None, base_url=None, prompt=True):
    ...
    base = validate_base_url(
        base_url
        or os.environ.get("JV_API_BASE_URL")
...
def _password():
    ...
    if value is None:
        if not sys.stdin.isatty():
            raise JvError(
                "For noninteractive use, supply JV_API_PASSWORD through a trusted secret source"
            )
        with warnings.catch_warnings():
            ...
            try:
                ...
                raise JvError(
                    "Cannot hide the password on this terminal; refusing an echoed password prompt"
                ) from None
    if not value:
...
def _logout(client):
    try:
        ...
        say(
            "Warning: token cleared locally, but server revocation could not be confirmed"
        )
...
def _find_engine():
    ...
    if explicit:
        ...
        return str(path)
    for path in (
        LOCAL_RUNTIME / "node_modules/.bin/codex",
        LOCAL_RUNTIME / "bin/codex",
    ):
...
def _toml(value):
    ...
    if isinstance(value, dict):
        return (
            "{ "
            + ", ".join(_toml(k) + " = " + _toml(v) for k, v in value.items())
            + " }"
        )
...
def _model_catalog(structured=False):
    ...
    return {
        "models": [
            {
                "slug": "jv-local",
                "display_name": "JV Local",
                "description": "JV job API adapter",
                ...
                "tool_mode": "direct",
                "base_instructions": STRUCTURED_AGENT_INSTRUCTIONS
                if structured
                else BASE_AGENT_INSTRUCTIONS,
            }
        ]
    }
...
def _write_engine_config(
    ...
    agent_instructions = (
        STRUCTURED_AGENT_INSTRUCTIONS if structured else BASE_AGENT_INSTRUCTIONS
    )
    ...
    atomic_write(
        engine_home / "config.toml",
        "# Managed by JV CLI; no password or token stored here.\n"
...
def _engine_env(session_dir=None, adapter_key=None):
    ...
    if session_dir:
        env.update(
            {
                "CODEX_HOME": str(session_dir / "engine"),
                "HOME": str(session_dir / "tool-home"),
                "TMPDIR": str(session_dir / "tmp"),
                "XDG_CACHE_HOME": str(session_dir / "tool-home/.cache"),
            }
        )
    if adapter_key:
...
def _command_preview(command, width=100):
    ...
    try:
        ...
        if (
            len(words) == 3
            and words[0] in ("/bin/bash", "/bin/sh", "bash", "sh")
            and words[1] in ("-lc", "-c")
        ):
            command = words[2]
    except ValueError:
    ...
    if shortened:
        first += (
            f" [{len(lines)} lines; --verbose for details]"
            if len(lines) > 1
            else " [--verbose for details]"
        )
    return first
...
class _ProgressDisplay:
    ...
    def update(self, status, now, verbose=False):
        ...
        if now - self.last_emit >= (15 if verbose else 60):
            ...
            return (
                f"Still waiting ({elapsed // 60}m {elapsed % 60:02d}s elapsed): {status}"
                if verbose
                else f"Still waiting ({elapsed // 60}m {elapsed % 60:02d}s elapsed)."
            )
        return None
    ...
    if kind == "agent_message" and not started:
        ...
        if isinstance(text, str) and text.strip():
            ...
            if sys.stdout.isatty():
                text = _format_answer(
                    text, shutil.get_terminal_size((88, 24)).columns - 2
                )
            ...
            if verbose or code not in (0, "0", None):
                ...
                if isinstance(output, str):
                    if output.strip():
                        say(
                            "  Output"
                            + (" (last 4000 characters)" if len(output) > 4000 else "")
                            + ":\n"
                            + output[-4000:]
                        )
        ...
        names = [
            str(c.get("path", "file")) for c in changes[:12] if isinstance(c, dict)
        ]
...
def _run_engine(
    ...
    *,
    ...
    process = subprocess.Popen(
        command,
        env=_engine_env(session_dir, runtime.key if runtime else None),
        ...
        while True:
            ...
            if time.monotonic() - started >= turn_timeout:
                if runtime:
                    runtime.cancel.set()
                say(
                    "Turn timed out. Local agent stopped; an already submitted remote job may continue."
                )
                forced_rc = 124
                break
            try:
                ...
                if time.monotonic() - last_progress >= 15:
                    status = (
                        "local command"
                        if command_active
                        else (runtime.status if runtime else "agent engine")
                    )
            ...
            if json_mode:
                ...
                print(
                    json.dumps(redact_data(event, secrets), ensure_ascii=False),
                    flush=True,
                )
            typ = event.get("type")
            if typ == "thread.started" and isinstance(event.get("thread_id"), str):
                ...
                if isinstance(item, dict):
                    ...
                    if not json_mode:
                        ...
                        _describe_item(
                            item, started=typ == "item.started", verbose=verbose
                        )
    ...
    if rc == 0 and not failed and (not completed or not saw_message):
        say(
            "Agent ended without a confirmed final answer. This is not reported as success."
        )
        failed = True
    if (
        rc == 0
        and runtime
        and isinstance(runtime.processor, AgentProcessor)
        and runtime.processor.last_completion
        not in ("committed", "conversation_complete")
    ):
        say("JV-AGENT-INCOMPLETE: no accepted task completion commitment.")
        failed = True
...
def _workspace_check(path):
    ...
    if (
        ...
        or path == user_home
        or path == APP_ROOT
        or path in APP_ROOT.parents
    ):
        raise JvError(
            "Choose a dedicated project directory, not your home, filesystem root, or the JV CLI installation"
        )
    ...
    for parent in (path, *path.parents):
        ...
        if (parent / ".codex/config.toml").exists():
            raise JvError(
                f"Existing agent project config found at {parent / '.codex/config.toml'}. Review it and use a clean test copy; this release does not load custom project engine config"
            )
    return path
...
def _run_session(
    ...
    *,
    ...
    if not engine:
        raise JvError(
            "Local agent engine is missing. From the JV CLI folder run ./install.sh"
        )
    actual = _version_of_engine(engine)
    if actual != ENGINE_VERSION:
        raise JvError(
            f"Engine {actual} is not the pinned {ENGINE_VERSION}; run ./install.sh. No automatic downgrade is performed at runtime"
        )
    base, user = _resolve_account()
    structured = _structured_enabled()
    if images and not structured:
        raise JvError(
            "--image requires JVCLI_AGENT_API=1; legacy ask --file remains available"
        )
    ...
    if resume and (
        ...
        or metadata.get("username") != user
        or metadata.get("base_url") != base
    ):
        raise JvError(
            "Resume must use the same project directory, API origin and username as the saved session"
        )
    if resume and metadata.get("transport_mode", "legacy") != transport_mode:
        raise JvError(
            "Resume must use the same legacy/structured transport mode as the saved session"
        )
    private_dir(session_dir)
    install_fd = os.open(
        STATE_DIR / "install.lock", os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600
    )
    try:
        ...
        os.close(install_fd)
        raise JvError(
            "Installation/update is in progress; do not start a session yet"
        ) from None
    try:
        lock_fd = os.open(
            session_dir / "session.lock", os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600
        )
    ...
    try:
        try:
            ...
            raise JvError(
                "This JV session is already open in another terminal"
            ) from None
        max_requests = int(
            positive_number(
        ...
        image_snapshots = snapshot_images(
            images, workspace, session_dir / "images" / uuid.uuid4().hex
        )
        client, user = _login_client(user, base)
        try:
            processor = (
                AgentProcessor(client, session_dir / "structured")
                if structured
                else None
            )
        except Exception:
        ...
        try:
            port = runtime.start()
            overrides = _write_engine_config(
                session_dir, port, read_only, allow_network, structured=structured
            )
            atomic_write(
                session_dir / "session.json", json.dumps(metadata, indent=2) + "\n"
            )
            say(
                f"JV CLI {VERSION}\nWorkspace: {workspace}\nSession: {sid}\nSandbox: {'read-only' if read_only else 'workspace-write'}; tool network: {'enabled' if allow_network else 'disabled'}"
            )
            say(
                "Only use trusted projects. The selected workspace may be changed; installation isolation is not a VM."
            )
            if prompt is None:
            ...
            while True:
                ...
                if one_shot:
                    ...
                    if task == "/new":
                        if runtime.lock.locked():
                            say(
                                "Previous request is still stopping. Exit and restart before submitting another task."
                            )
                        else:
                            thread_id = None
                            say(
                                "New agent thread. Saved earlier engine histories are retained locally."
                            )
                        continue
                    if task.startswith("/"):
                ...
                if rc:
                    say(
                        "Turn stopped. Check /status and the reported JV job before resubmitting a task."
                    )
        finally:
            runtime.close()
            _logout(client)
...
def command_login(args):
    ...
    try:
        say(
            f"Credentials verified for {user}. Username/origin saved; password and bearer token are not saved."
        )
    finally:
        _logout(client)
    return 0
...
def command_doctor(json_mode=False):
    ...
    try:
        ...
        config = _load_disk_config()
        data["base_url"] = validate_base_url(
            os.environ.get("JV_API_BASE_URL")
            or config.get("base_url")
            or DEFAULT_BASE_URL
        )
        data["username"] = (
            os.environ.get("JV_API_USERNAME")
        ...
        engine = _find_engine()
        data["engine_version"] = (
            _version_of_engine(engine) if engine else "NOT INSTALLED"
        )
    ...
    if json_mode:
        ...
        if "error" in data:
            say(data["error"])
        print(
            "Doctor does not contact the live API or prove sandbox enforcement. Run the acceptance checks before rollout."
        )
...
def command_raw(args):
    ...
    try:
        ...
        downloads = (
            client.download_response_files(job, Path(args.download_dir))
            if args.download_dir and job["status"] == "succeeded"
            else []
        )
        if args.json:
            # Whitelist public fields; do not print arbitrary server response keys.
            safe = {
                k: job.get(k) for k in ("id", "conversation_id", "status", "answer")
            }
            safe["downloaded_files"] = [str(p) for p in downloads]
            print(
                json.dumps(
                    redact_data(safe, (client._token or "",)),
                    ensure_ascii=False,
                    indent=2,
                )
            )
            ...
            print(
                terminal_text(
                    redact(
                        job.get("answer") or "(No text answer)", (client._token or "",)
                    )
                )
            )
...
def command_sessions():
    ...
    for path in sorted(directory.glob("*/session.json")):
        ...
        print(
            terminal_text(
                f"{value.get('session_id', path.parent.name)}  {value.get('workspace', '?')}"
            )
        )
    return 0
...
def _parser():
    parser = argparse.ArgumentParser(
        prog="jvcli", description="JV CLI coding agent. No password is stored."
    )
    parser.add_argument("--version", action="version", version="JV CLI " + VERSION)
    parser.add_argument(
        "--read-only", action="store_true", help="Deny model tool writes"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Show full commands and bounded tool output for diagnostics",
    )
    ...
    login = subs.add_parser(
        "login", help="Verify credentials and save username/API origin only"
    )
    ...
    subs.add_parser(
        "logout",
        help="Forget saved account settings; active sessions must be exited separately",
    )
    ...
    uninstall = subs.add_parser(
        "uninstall", help="Safely remove the per-user JV CLI installation"
    )
    uninstall.add_argument(
        "--keep-state",
        action="store_true",
        help="Preserve local account settings and sessions",
    )
    uninstall.add_argument(
        "--yes",
        action="store_true",
        help="Confirm the displayed removal warning noninteractively",
    )
    for command in ("exec", "resume"):
        ...
        network.add_argument(
            "--allow-network", action="store_true", default=argparse.SUPPRESS
        )
        network.add_argument(
            "--no-network",
            action="store_false",
        ...
        sub.add_argument(
            "--json",
            action="store_true",
            help="Output agent events as JSONL, diagnostics on stderr",
        )
        sub.add_argument(
            "--image",
            action="append",
    ...
    for command in ("ask", "job"):
        sub = subs.add_parser(
            command, help="Direct JV API request; does not execute coding tools"
        )
        if command == "ask":
...
def main(argv=None):
    ...
    try:
        ...
        if prompt is None and not sys.stdin.isatty():
            raise JvError(
                'Interactive mode requires a terminal; use jvcli exec "your task" for automation'
            )
        return _run_session(
            prompt,
            resume=getattr(args, "session_id", None),
